<div>
    <h1 class="text-4xl py-6 font-bold h-24 px-8">Tweets</h1>
    ...
    <form method="post" wire:submit.prevent="create" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-8">
        ...
        @enderror
        <button type="submit" class="bg-blue-500 hover:bg-blue-400 text-white p-2 pl-6 pr-6 rounded">Tweetar</button>
    </form>
    ...
    @foreach ($tweets as $tweet)
        <div class="flex m-8 bg-white shadow-md rounded p-4">
            <div class="w-1/8 pl-3 text-center">
                ...
                @else
                    <img src="{{ url('image/usuario.png') }}" alt="{{ $tweet->user->name }}" class="rounded-full h-8 w-8">
                @endif
                {{-- $tweet->user->name --}}
            </div>
